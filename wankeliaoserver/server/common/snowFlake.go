...
const (
	...
	epoch       int64 = 1566202889901           // 起始時間戳，這個是我在寫epoch這個常量時的時間戳(毫秒)
)
...
func (u UUID) Hexstring() string {
	...
	return strings.Repeat("0", 16-len(idstr)) + idstr
}

func Addzerohexstring(u int64) string {
	idstr := strconv.FormatInt(int64(u), 16)
	return strings.Repeat("0", 16-len(idstr)) + idstr
}
